...
window_x, window_y = 256, 256  # Starting point of the window
window_size = 32
...
for image_file in image_files:
    ...
    image = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)  # Read without converting to grayscale
    if image is None:
        print(f"Failed to read image: {image_file}")
        continue

    # Downsample from 16-bit to 12-bit if necessary
    if image.dtype == np.uint16:  # Check if image is 16-bit
        image = (image >> 4).astype(np.uint16)  # Right shift by 4 bits to reduce from 16-bit to 12-bit
...
aggregate_noise = np.std(np.concatenate(pixel_data))  # Compute global noise as std dev of all pixels
